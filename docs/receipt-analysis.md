...
5. The Edge Function verifies the caller and sends the OCR text to Gemini for categorization and line-item extraction.
6. If local OCR is unavailable, the function falls back to a short-lived signed image URL and Gemini vision.
...
If the function is not deployed or Gemini fails, the app falls back to the manual receipt-attached expense form.
