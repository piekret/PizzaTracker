...
begin
  ...
  v_next_reset := (v_period_month + interval '1 month')::date +
    (v_reset_day - 1);
  ...
  v_remaining_budget := (v_disposable_budget - v_spent_this_period)
    ::numeric(10,2);
  ...
  v_elapsed_days := (p_on_date - v_period_start)::integer + 1;
  if v_elapsed_days < 1 then v_elapsed_days := 1; end if;

  v_daily_limit := round(
    (v_remaining_budget / greatest(v_days_left, 1))::numeric,
    2
  );
  ...
  if v_disposable_budget > 0 and v_ideal_daily > 0 then
    ...
    v_schedule_pressure := greatest(
      least(((v_expected_remaining - v_remaining_budget) /
        v_disposable_budget), 1),
      0
    );

    if v_remaining_budget < 0 then
      v_over_budget_pressure := greatest(
        (-v_remaining_budget) / v_disposable_budget,
        0
      );
    end if;
